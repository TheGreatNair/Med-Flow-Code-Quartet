...
class PatientPriorityQueue:
    ...
    def refresh(self, current_time):
        ...
        for entry in self.queue:
            new_priority = self.calculate_priority(entry.patient, current_time)
            refreshed.append(
                QueueEntry(
...
class HospitalSimulation:
    ...
    def walk_in_arrival_rate(self, time):
        hour = (time // 60) % 24
        if self.is_surge_period(time): return 18
        if 0 <= hour < 6: return 4
        if 6 <= hour < 12: return 10
        if 12 <= hour < 18: return 12
        return 14

    def ambulance_arrival_rate(self, time):
        hour = (time // 60) % 24
        if self.is_surge_period(time): return 6
        if 0 <= hour < 6: return 1
        if 6 <= hour < 12: return 2
        if 12 <= hour < 18: return 3
        return 4
    ...
    def generate_arrivals(self, time):
        walk_in_count = self.poisson_count(self.walk_in_arrival_rate(time))
        ambulance_count = self.poisson_count(self.ambulance_arrival_rate(time))

        for _ in range(walk_in_count):
            self.create_patient(arrival_time=time, arrival_mode="walk-in")

        for _ in range(ambulance_count):
            self.create_patient(arrival_time=time, arrival_mode="ambulance")

    def choose_urgency(self, arrival_mode):
        if arrival_mode == "ambulance":
            return random.choices(
                population=[Urgency.CRITICAL, Urgency.URGENT, Urgency.NORMAL, Urgency.LOW],
    ...
    def create_patient(self, arrival_time, arrival_mode):
        ...
        needs_icu = (urgency == Urgency.CRITICAL or random.random() < 0.08)
        required_staff = "doctor" if urgency in [Urgency.CRITICAL, Urgency.URGENT] else "nurse"
    ...
    def schedule_random_failure(self):
        failure_time = random.randint(60, self.simulation_minutes - 120)
        resource_type = random.choices(
            population=["doctor", "nurse", "icu_bed"],
            weights=[0.40, 0.35, 0.25]
        )[0]

        failed_units = 1 if resource_type in ["doctor", "icu_bed"] else 2
        duration = random.randint(30, 120)

        self.schedule_event(
            event_time=failure_time,
            event_type="resource_failure_start",
            data={"resource_type": resource_type, "failed_units": failed_units, "duration": duration}
        )
    ...
    def start_resource_failure(self, failure_data):
        ...
        print(f"[{self.format_time(self.current_time)}] FAILURE: {failed_units} {resource_type} resource(s) unavailable for {duration} minutes")
    ...
    def end_resource_failure(self, failure_data):
        ...
        self.resource_failures[resource_type] = max(
            0, self.resource_failures[resource_type] - failed_units
        )
        ...
        print(f"[{self.format_time(self.current_time)}] RECOVERY: {resource_type} resource(s) restored")

    def apply_staff_shortage(self):
        self.resources.available_staff["doctor"] = max(1, self.resources.available_staff["doctor"] - 2)
        self.resources.available_staff["nurse"] = max(1, self.resources.available_staff["nurse"] - 3)

    def restore_staff(self):
        self.resources.available_staff = self.resources.total_staff.copy()

    def start_treatment(self):
        while True:
            patient = self.waiting_queue.pop_best_available(
                current_time=self.current_time,
                can_treat=lambda p: self.resources.can_treat(p, self.resource_failures)
            )

            if patient is None:
                break

            if not self.resources.can_treat(patient, self.resource_failures):
                break
            ...
            finish_time = self.current_time + patient.treatment_time
    ...
    def report(self):
        ...
        average_wait = sum(completed_waiting_times) / len(completed_waiting_times) if completed_waiting_times else 0
        max_wait = max(completed_waiting_times) if completed_waiting_times else 0
        ...
        return {
            ...
            "resource_failure_events": self.metrics["resource_failure_events"],
            "failure_downtime_minutes": self.metrics["failure_downtime_minutes"],
